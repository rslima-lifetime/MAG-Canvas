...
const Insight_Box: React.FC<Insight_BoxProps> = ({ content, limit = 400 }) => {
  ...
  return (
    <div className="flex flex-col h-full bg-[#f8fafc] p-6 border border-[#e2e8f0] relative">
      <h3 className="text-[#006098] font-bold text-base mb-3 uppercase flex items-center gap-2">
        <span className="w-1.5 h-6 bg-[#00A7E7] block"></span>
        Análise Qualitativa e Insights
      </h3>
      
      <div className="text-[#415364] text-base leading-relaxed whitespace-pre-wrap">
        {content || "Nenhuma análise inserida."}
      </div>

      {isOverLimit && (
        <div className="absolute bottom-2 right-4 text-xs text-rose-500 font-bold uppercase">

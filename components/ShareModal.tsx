      ...
      <div className="bg-white rounded-3xl max-w-md w-full p-8 shadow-2xl animate-in zoom-in-95 duration-200 relative border border-slate-200">
        ...
        <div className="flex items-center gap-3 mb-6">
          ...
          <div>
            ...
            <p className="text-xs text-slate-400 font-bold uppercase tracking-widest">Defina quem pode acessar seu arquivo</p>
          </div>
        </div>
        ...
        <div className="space-y-4 mb-8">
          ...
          >
            ...
            <div className="flex flex-col items-start">
              ...
              <span className="text-[11px] font-medium opacity-60">Apenas você pode ver e editar</span>
            </div>
          ...
          >
            ...
            <div className="flex flex-col items-start">
              ...
              <span className="text-[11px] font-medium opacity-60">Restrito às equipes que você criou</span>
            </div>
          ...
          {isShared && (
            <div className="pl-4 pr-2 py-2 bg-slate-50 rounded-2xl border border-slate-200 mt-2 animate-in slide-in-from-top-2">
              <label className="text-[10px] font-black text-slate-400 uppercase tracking-wider ml-1">Selecionar Equipe</label>
              {loading ? (
                <div className="text-xs text-slate-400 p-2">Carregando equipes...</div>
              ) : teams.length === 0 ? (
                <div className="text-xs text-slate-400 p-2 italic">Nenhuma equipe encontrada. Crie uma na aba 'Equipes' da tela inicial.</div>
              ) : (
                <select 
                  value={selectedTeamId || ''} 
                  onChange={(e) => setSelectedTeamId(e.target.value || null)}
                  className="w-full bg-transparent border-none text-sm font-bold text-[#006098] outline-none cursor-pointer py-1 font-sans"
                >
                  {teams.map(t => (
          ...
          >
            ...
            <div className="flex flex-col items-start">
              ...
              <span className="text-[11px] font-medium opacity-60">Qualquer pessoa com o link pode visualizar</span>
            </div>
            {isShared && !selectedTeamId && <Check size={16} className="ml-auto text-[#0079C2]" />}
          </button>
        </div>
        ...
        {isShared && projectId && (
          <div className="p-4 bg-slate-50 rounded-2xl border border-slate-200 flex items-center justify-between mb-6">
            <div className="truncate flex-1 pr-4 text-xs font-bold text-slate-500 font-mono">
              {`${window.location.origin}${window.location.pathname}?p=${projectId}`}
            </div>
            <button 
              onClick={copyShareLink}
              className={`px-4 py-2 rounded-xl text-[11px] font-black uppercase tracking-widest transition-all ${linkCopied ? 'bg-emerald-500 text-white' : 'bg-[#0079C2] text-white hover:bg-[#006098]'}`}
            >
        ...
        <div className="flex gap-3">
          <button 
            onClick={onClose}
            className="flex-1 py-3.5 rounded-2xl font-black uppercase tracking-widest text-xs text-slate-400 bg-slate-100 hover:bg-slate-200 transition-all"
          >
            Cancelar
          </button>
          <button 
            onClick={handleSave}
            className="flex-1 py-3.5 rounded-2xl font-black uppercase tracking-widest text-xs text-white bg-[#0079C2] hover:bg-[#006098] shadow-lg transition-all active:scale-95"
          >
            Salvar Acesso
          </button>
        </div>
